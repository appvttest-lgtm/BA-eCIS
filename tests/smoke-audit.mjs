...
const eparcelAudit = auditLabel({
  ...
  extractedText: [
    'Parcel Post', 'TO:', 'MR C RECEIVER', 'Unit 14', '1 Test Street', 'CHULLORA NSW 2190',
    'FROM: MR C SENDER', '1 Jedda Road', 'PRESTONS NSW 2170',
    'Cons No: JDQ0194571', 'AP Article ID: JDQ0 1945 7101 0009 3030 8',
    'Aviation Security and Dangerous Goods Declaration',
    'The sender acknowledges that this article may be carried by air',
    '0.5kg', '0609'
  ].join('\n')
});
expect('variant resolves to parcel-post', eparcelAudit.ruleSet?.variant === 'parcel-post', `got ${eparcelAudit.ruleSet?.variant}`);
...
expect('EP-LIN-07 GTIN prefix', find(eparcelAudit, 'EP-LIN-07')?.status === 'pass', find(eparcelAudit, 'EP-LIN-07')?.message);
expect('EP-ART-06 check digit', find(eparcelAudit, 'EP-ART-06')?.status === 'pass', find(eparcelAudit, 'EP-ART-06')?.message);
expect('EP-DM-05 AI 420 postcode', find(eparcelAudit, 'EP-DM-05')?.status === 'pass');
expect('EP-TO-08 postcode matches TO address', find(eparcelAudit, 'EP-TO-08')?.status === 'pass', find(eparcelAudit, 'EP-TO-08')?.message);
expect('EP-DM-07 AI 8008 datetime', find(eparcelAudit, 'EP-DM-07')?.status === 'pass');
expect('EP-LIN-09 linear/DM agreement', find(eparcelAudit, 'EP-LIN-09')?.status === 'pass', find(eparcelAudit, 'EP-LIN-09')?.message);
expect('EP-SVC-03 service/product matrix', find(eparcelAudit, 'EP-SVC-03')?.status === 'pass', find(eparcelAudit, 'EP-SVC-03')?.message);
expect('EP-SVC-07 product allowed for parcel post', find(eparcelAudit, 'EP-SVC-07')?.status === 'pass');
expect('EP-TO-06 suburb line capitalised', find(eparcelAudit, 'EP-TO-06')?.status === 'pass', find(eparcelAudit, 'EP-TO-06')?.message);
expect('rule metadata attached for report UI', Boolean(find(eparcelAudit, 'EP-DM-05')?.rule?.logic && find(eparcelAudit, 'EP-DM-05')?.input));
...
const qrPayload = [
  pad('CHULLORA', 30), pad('2190', 4), pad('ABCD12345678', 12), pad('ABCD12345678EXP00001', 20),
  pad('EXP', 3), pad('', 8), pad('12345678', 8), pad('1', 4), pad('5', 5), pad('15', 5),
  pad('20260610', 8), pad('CAROL RECEIVER', 40), pad('', 40), pad('CTN', 3), pad('SYD', 4),
  pad('8 TEST CLOSE', 40), pad('', 40), pad('', 14), 'N', 'N', pad('', 12), pad('', 12), pad('', 10), pad('', 10)
].join('');
const startrackAudit = auditLabel({
  ...
  extractedText: ['STARTRACK', 'EXP', 'CONNOTE: ABCD12345678', 'CHULLORA NSW 2190', 'ITEM 1 OF 1', '5 kg', '0.015 m3'].join('\n')
});
expect('variant resolves to express', startrackAudit.ruleSet?.variant === 'express', `got ${startrackAudit.ruleSet?.variant}`);
...
expect('ST-RTE-03 routing/product compatibility', find(startrackAudit, 'ST-RTE-03')?.status === 'pass', find(startrackAudit, 'ST-RTE-03')?.message);
expect('ST-RTE-04 routing postcode matches QR', find(startrackAudit, 'ST-RTE-04')?.status === 'pass', find(startrackAudit, 'ST-RTE-04')?.message);
expect('ST-X-01 QR connote matches freight', find(startrackAudit, 'ST-X-01')?.status === 'pass', find(startrackAudit, 'ST-X-01')?.message);
expect('ST-X-02 QR freight item matches barcode', find(startrackAudit, 'ST-X-02')?.status === 'pass', find(startrackAudit, 'ST-X-02')?.message);
