...
> **Live coverage:** the `Coverage` column below is descriptive and may lag the code (several rows marked ❌/🟡 are now implemented). The authoritative intended-vs-actual status is generated by `npm run checklist` from [`intended-checklist.json`](intended-checklist.json) (the machine-readable source of truth); see also the [validation matrix](validation-matrix.md).
